...
export default defineEventHandler(async (event) => {
  ...
  if (["JWT_SECRET", "G_CLIENT_ID", "G_CLIENT_SECRET"].find(k => !process.env[k])) {
    ...
    if (["/api/s", "/api/proxy", "/api/latest", "/api/mcp", "/api/spotlight", "/api/livefeed", "/api/ai", "/api/enable-login", "/api/read", "/api/login", "/api/callback"].every(p => !url.pathname.startsWith(p)))
